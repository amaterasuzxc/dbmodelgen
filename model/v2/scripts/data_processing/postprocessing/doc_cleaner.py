...
class Cleaner:
    def __init__(self, config_path: str) -> None:
        self._config = yaml_reader.read(config_path)
    ...
    def _resolve_new_spans(self, doc: Doc, all_spans: List[Tuple[Span, Span]]) -> Doc:
        ...
        new_spans = list(filter(lambda _:_ is not None, [new_span for (_, new_span) in all_spans]))
        for (old_span, new_span) in all_spans:
            if new_span is None:
                categories.pop(old_span.start)
                relations.pop((old_span.start, any))
                relations.pop((any, old_span.start))
            if new_span.start != old_span.start:
                categories[new_span.start] = categories.pop(old_span.start)
                relations[(new_span.start, any)] = relations.pop((old_span.start, any))
                relations[(any, new_span.start)] = relations.pop((any, old_span.start))
        doc.ents = new_spans
        return doc
    ...
    def clean(self, doc: Doc) -> Doc:
        ...
        old_spans = new_spans = temp_doc.ents
        ...
        all_spans = zip(old_spans, new_spans)
